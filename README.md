...
### If you have a [Claude Pro or Max](https://claude.ai/upgrade) subscription

```bash
# If already logged into Claude Code, just:
aoud start --no-auth
...
### If you have an [API billing](https://console.anthropic.com/) account

```bash
ANTHROPIC_API_KEY=sk-ant-xxxxx aoud start --no-auth
```

> Use `--no-auth` to skip the aoud authentication token prompt for quick local use.
...
Contributions, issues, and feedback are welcome! See [CONTRIBUTING.md](CONTRIBUTING.md).
