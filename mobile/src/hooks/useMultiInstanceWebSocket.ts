...
export function useMultiInstanceWebSocket(enabled: boolean = true) {
  ...
  const fetchInstances = useCallback(async () => {
    try {
      ...
      log.error('MultiInstanceWebSocket', 'Failed to fetch instances:', error);
  ...
  const connectToInstance = useCallback((instance: BridgeInstance) => {
    ...
    if (connecting.current.has(instance.instanceId)) {
      log.debug('MultiInstanceWebSocket', `Already connecting to ${instance.projectName}, skipping duplicate`);
      return;
    }
    ...
    log.info('MultiInstanceWebSocket', `Connecting to ${instance.projectName} on port ${instance.port}`);
    ...
    ws.onopen = () => {
      ...
      log.success('MultiInstanceWebSocket', `Connected to ${instance.projectName}`);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        log.error('MultiInstanceWebSocket', 'Error parsing message:', error);
      }
    };

    ws.onerror = (error) => {
      connecting.current.delete(instance.instanceId);
      log.error('MultiInstanceWebSocket', `WebSocket error for ${instance.projectName}:`, error);
    };

    ws.onclose = () => {
      connecting.current.delete(instance.instanceId);
      const instanceId = (ws as any)._instanceId;
      log.warn('MultiInstanceWebSocket', `Disconnected from ${instance.projectName}`);
  ...
  const sendToInstance = useCallback((instanceId: string, message: any) => {
    ...
    log.warn('MultiInstanceWebSocket', `Cannot send message - instance ${instanceId} not connected`);
    return false;
