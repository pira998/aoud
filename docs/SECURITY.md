...
This document outlines security considerations and best practices for Claude Mobile Bridge.

## Security Model

Claude Mobile Bridge is designed for **single-user, trusted network environments**. The bridge server runs on your local machine and connects to your mobile device to control Claude Code sessions.
...
- Regenerate tokens periodically: `npx claude-bridge config --reset-token`
...
#### TLS/HTTPS Support
- Optional TLS mode: `npx claude-bridge start --tls`
...
- Config file: `~/.claude-mobile-bridge/config.json`
...
# Start with TLS
npx claude-bridge start --tunnel --tls
...
If you discover a security vulnerability in Claude Mobile Bridge:

**DO NOT open a public GitHub issue.**

Instead, please email security concerns to: [YOUR_EMAIL]
...
Before deploying Claude Mobile Bridge:
