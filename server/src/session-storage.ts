...
export class SessionStorage {
  ...
  private getProjectDir(projectId: string): string {
    const dir = path.join(SESSIONS_DIR, 'by-project', projectId);
  ...
  cleanupIncompleteSessions(): void {
    try {
      ...
      if (active.sessions.length > 0) {
        console.log(`Found ${active.sessions.length} interrupted session(s), marking as interrupted...`);
        ...
        for (const activeSession of active.sessions) {
          this.updateSession(activeSession.sessionId, {
            ...
            console.error(`Error marking session ${activeSession.sessionId} as interrupted:`, err);
          });
        }
