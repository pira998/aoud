...
export class InstanceRegistryManager {
  ...
  private load(): InstanceRegistry {
    try {
      ...
      log.error('Instance Registry', 'Error loading registry:', error);
      log.warn('Instance Registry', 'Registry corrupted, resetting to empty state');
    }
  ...
  private save(registry: InstanceRegistry): void {
    try {
      ...
      log.error('Instance Registry', 'Error saving registry:', error);
  ...
  cleanupStaleInstances(): number {
    ...
    registry.instances = registry.instances.filter((instance) => {
      ...
      if (!isAlive) {
        log.info(
          'Instance Registry',
          `Removing stale instance: ${instance.projectName} (PID ${instance.pid}, port ${instance.port})`
        );
      }
      return isAlive;
    });
