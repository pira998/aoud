...
export class ProcessMonitor {
  ...
  start(): void {
    ...
    process.on('uncaughtException', (error) => {
      log.error('ProcessMonitor', 'Uncaught exception:', error);
      this.handleShutdown('uncaughtException');
    });

    // Handle unhandled promise rejections
    process.on('unhandledRejection', (reason, promise) => {
      log.error('ProcessMonitor', 'Unhandled promise rejection:', reason);
  ...
  private handleShutdown(signal: string): void {
    ...
    log.warn('ProcessMonitor', `Received ${signal}, shutting down gracefully...`);
    ...
    setTimeout(() => {
      log.success('ProcessMonitor', 'Shutdown complete');
      process.exit(0);
    }, 1000);
  }
  ...
  private cleanup(): void {
    try {
      ...
      if (wasUnregistered) {
        log.success('ProcessMonitor', `Instance ${this.instanceId} unregistered successfully`);
      }
    } catch (error) {
      log.error('ProcessMonitor', 'Error during cleanup:', error);
    }
  }
